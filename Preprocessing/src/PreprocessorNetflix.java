 ...
 */
public class PreprocessorNetflix {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        if (args.length != 1) {
            System.err.println("Usage: PreprocessorNetflix <input>");
            System.exit(1);
        }
        ...
        Job job = new Job(conf, "PreprocessorNetflix");
        job.setJarByClass(PreprocessorNetflix.class);
...
//        job.setInputFormatClass(TextInputFormat.class);
        ...
        job.getConfiguration().set(TableOutputFormat.OUTPUT_TABLE, TableConts.TABLE_USR_MOV_MAPPING);
    ...
    private static void createHbaseTable(Configuration conf) throws IOException {
        ...
        HTableDescriptor hd = new HTableDescriptor(TableConts.TABLE_USR_MOV_MAPPING);
        hd.addFamily(new HColumnDescriptor(TableConts.TABLE_USR_MOV_COL_FAMILY));
        HBaseAdmin admin = new HBaseAdmin(co);

        if(admin.tableExists(TableConts.TABLE_USR_MOV_MAPPING)){
            admin.disableTable(TableConts.TABLE_USR_MOV_MAPPING);
            admin.deleteTable(TableConts.TABLE_USR_MOV_MAPPING);
        }
    ...
    public static class PreReducer extends Reducer<Text, Text, ImmutableBytesWritable, Writable>{
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            super.setup(context);
            mTable = new HTable(HBaseConfiguration.create(), TableConts.TABLE_USR_MOV_MAPPING);
